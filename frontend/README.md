...
Nueve superficies cubren el local, en dos grupos del panel. **Operación**: los
cuatro puestos por los que pasa una comanda —`/pos` la toma, `/kds` la cocina,
`/caja` la cobra y la cierra, y `/despacho` la entrega al conductor de la empresa
de reparto—. **Gestión**: `/menu` (categorías, platillos y recetas),
`/inventario` (insumos, kardex y conteo), `/reportes` (ventas e indicadores),
`/personal` (quién trabaja y qué abre su cargo) y `/configuracion`
(parámetros, bots y eventos). El recorrido de una comanda es el mismo que documenta
...
| 9 | **Cada superficie es un chunk aparte.** El celular del mozo no descarga el arqueo de caja: las doce rutas con componente son `loadComponent`. | [app.routes.ts](src/app/app.routes.ts) · tabla de compilación más abajo |
...
| [menu.page.ts](src/app/paginas/menu/menu.page.ts) | ALMACEN, ADMIN | Categorías, platillos y recetas | Ver abajo | `CatalogoCategorias`, `CatalogoPlatillos`, `InventarioInsumos` |
...
| [reportes.page.ts](src/app/paginas/reportes/reportes.page.ts) | ADMIN, CAJA | Venta del rango, la serie por horas o días y lo que pide atención ahora | Manda las fechas en ISO **con desfase local**, no en `Z`: con UTC, la cena del sábado en Lima aparece repartida entre sábado y domingo | `Reportes` |
...
27 y 43 líneas; `menu` e `inventario` también, porque solo envuelven su
sección. Las secciones de menú, inventario y configuración, el personal y los
reportes comparten una sola hoja, [secciones.scss](src/app/disenio/secciones.scss),
por la misma razón por la que `.bloque` y `.chip` viven en `src/estilos/`:
varias copias del mismo formulario se desincronizan.
...
Las dos primeras envuelven una sola sección; la configuración reparte tres en
pestañas, y el `@switch` las monta y desmonta: mirar un parámetro no descarga la
bandeja de eventos entera
([configuracion.page.html](src/app/paginas/configuracion/configuracion.page.html)).

| Sección | Roles | Qué hace | Detalle clave |
|---|---|---|---|
| [inventario](src/app/paginas/inventario/inventario.seccion.ts) | ALMACEN, ADMIN | Insumos, kardex, movimiento suelto y conteo físico | La cantidad va siempre en positivo: **el signo lo pone el motivo** ([:199](src/app/paginas/inventario/inventario.seccion.ts#L199)). El conteo solo envía las líneas escritas —un insumo no contado no es un insumo en cero— y deja los descuadres en pantalla después de aplicarlos ([:314](src/app/paginas/inventario/inventario.seccion.ts#L314)) |
| [menú](src/app/paginas/menu/carta.seccion.ts) | ALMACEN, ADMIN | Categorías, platillos y recetas | `PUT /platillos/{id}/receta` **reemplaza**, no parchea ([:283](src/app/paginas/menu/carta.seccion.ts#L283)). Un platillo sin receta se vende pero no descuenta nada, y el inventario se separa de la realidad sin que nadie lo note ([:25](src/app/paginas/menu/carta.seccion.ts#L25)) |
| [local](src/app/paginas/configuracion/local.seccion.ts) | ADMIN | Cuatro parámetros, satisfacción y registro de empresas | No son preferencias de pantalla: el umbral decide cuándo la caja emite cupón y el objetivo de cocina es la vara del KDS. Cambiar uno cambia lo que hacen tres superficies ([:30](src/app/paginas/configuracion/local.seccion.ts#L30)). El `PUT` reemplaza, así que los cuatro campos viajan siempre ([:145](src/app/paginas/configuracion/local.seccion.ts#L145)) |
...
4. `INICIO_POR_ROL` decide dónde aterriza. `ADMIN` va a `/pos`, porque es
   quien recorre el flujo principal entero y ese recorrido empieza tomando una
   comanda; `ALMACEN`, a `/inventario`. Una prueba comprueba que ningún rol
...
|---|---|---|
| **Inicial** (main + runtime + estilos + español) | 498,43 kB | 108,95 kB |
| `configuracion-page` | 48,37 kB | 7,59 kB |
| `login-page` | 47,10 kB | 11,45 kB |
| `pos-page` | 41,99 kB | 8,81 kB |
| `personal-page` | 29,41 kB | 6,08 kB |
| `pt` (diccionario) | 28,85 kB | 8,73 kB |
| `caja-page` | 28,75 kB | 6,52 kB |
| `en` (diccionario) | 27,88 kB | 8,26 kB |
| `inventario-page` | 23,30 kB | 5,29 kB |
| `menu-page` | 20,06 kB | 4,52 kB |
| `despacho-page` | 19,34 kB | 4,78 kB |
| `reportes-page` | 19,00 kB | 4,88 kB |
| `kds-page` | 16,55 kB | 4,17 kB |
| `confirmacion` (diálogo de peligro) | 4,72 kB | 1,74 kB |
| `sin-permiso-page` | 1,29 kB | 687 B |
...
El diálogo de confirmación tampoco: `ConfirmacionService` lo descarga con
`import()` la primera vez que alguien pide confirmar. Con `@defer` el runtime
añadido pesaba más que lo que sacaba del inicial.
...
```bash
pnpm test        # Vitest 4 + jsdom · 9 archivos, 54 pruebas, ~1,7 s
```

Las nueve suites cubren lo que falla en silencio, que es lo que no se ve al
mirar la pantalla:
