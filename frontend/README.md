...
alérgenos, complementos, promociones y la lectura de la carta desde fotos), `/inventario` (insumos con lotes y vencimientos, kardex, conteo y
...
| [menu.page.ts](src/app/paginas/menu/menu.page.ts) | ALMACEN, ADMIN | Platillos con su receta, foto, costo y alérgenos; complementos, promociones y el catálogo de alérgenos, en pestañas, y para ADMIN la lectura de la carta desde fotos | Ver abajo | `CatalogoCategorias`, `CatalogoPlatillos`, `CatalogoComplementos`, `CatalogoPromociones`, `CatalogoAlergenos`, `CatalogoLecturaDeCarta`, `Archivos`, `InventarioInsumos` |
...
| [leer carta](src/app/paginas/menu/lectura-carta.seccion.ts) | ADMIN | Subir hasta seis fotos de la carta impresa, revisar fila por fila lo que el servidor reconoció e importar solo lo marcado | **Leer no guarda nada**: la pantalla es la que decide qué entra. Cada fila dice por adelantado qué va a pasar —se crea, cambia de precio «S/ 20 → S/ 21» o ya está igual—, comparando el nombre sin tildes ni mayúsculas, que es la misma regla del servidor. Lo que la foto no dejó claro llega marcado como dudoso, **sin la casilla puesta** y con el recorte de la foto al lado del nombre. La sección leída se sugiere contra las que ya existen; si no coincide ninguna, se crea con el nombre leído. Cada sección tiene «Agregar fila» para lo que la foto se comió. Las fotos se reducen en el navegador antes de subirlas y se leen una detrás de otra, con el aviso de por cuál va |
...
6. `InactividadService` cierra la sesión tras **media hora sin actividad**, con
   la cuenta atrás a la vista los últimos cinco minutos
   ([aviso-inactividad.ts](src/app/disenio/aviso-inactividad.ts)) y un botón
   «Sigo aquí». Es para el terminal compartido: la caja y el POS se quedan
   abiertos entre cliente y cliente. **Cocina (`/kds`) y Despacho quedan
   exentas**, porque son paneles de pared que nadie toca en todo el turno y
   cerrarles la sesión dejaría la cocina sin comandas. La cuenta se lleva con la
   marca de tiempo del último gesto y no con un temporizador que se reinicia: un
...
Veintiuna pruebas cubren esa cadena, incluidos los casos que más duelen: el
...
el login que no debe verse con la sesión abierta
([guardas.spec.ts](src/app/nucleo/sesion/guardas.spec.ts)) y la cola de cocina
que no debe cerrarse sola
([inactividad.service.spec.ts](src/app/nucleo/sesion/inactividad.service.spec.ts)).
...
| [menu.page.spec.ts](src/app/paginas/menu/menu.page.spec.ts) | Que el menú ofrezca sus pestañas, que el almacén vea las promociones sin poder crearlas y que leer la carta desde fotos solo se le ofrezca al administrador |
| [lectura-carta.seccion.spec.ts](src/app/paginas/menu/lectura-carta.seccion.spec.ts) | Que sin lector en el servidor no se puedan elegir fotos, que cada fila anuncie si se crea o cambia de precio, que la dudosa llegue con su recorte y sin marcar, y que se importe solo lo marcado |
